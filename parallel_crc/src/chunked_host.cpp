...
int main(int argc, char **argv)
{
    ...
    while (rerun == "yes")
    {
        ...
        while (file_size >= 16)
        {
            ...
            //std::cout << "Chunk Size: " << std::dec << chunkSize << std::endl;
            //std::cout << "Chunk Size in blocks : " << std::dec << (chunkSize / 16) << std::endl;
            ...
            // Read the result
            OCL_CHECK(err, err = queue.enqueueReadBuffer(buffer_out, CL_TRUE, 0, sizeof(unsigned int), &output_data.data()[0]));
            start_check = std::chrono::high_resolution_clock::now(); 
        ...
        double multiplier = 1.0/(double)calc_time.count();
        std::cout << "Data Processed: " << dataProcessed << std::endl;
        double throuput = ((double)dataProcessed) * multiplier * 8;
        if (throuput > 1000000000.0) {
            throuput = throuput / 1000000000.0;
            std::cout << "Throughput (gbps): " << throuput << std::endl;
        } else if (throuput > 1000000.0) {
            throuput = throuput / 1000000.0;
            std::cout << "Throughput (mbps): " << throuput << std::endl;
        } else {
            std::cout << "Throughput (bps): " << throuput << std::endl;
        }
        /*
