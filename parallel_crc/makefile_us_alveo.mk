...
all: check-platform check-device check-vitis $(EXECUTABLE) $(BUILD_DIR)/kernel.xclbin emconfig

.PHONY: host
host: $(EXECUTABLE)

.PHONY: build
build: check-vitis check-device $(BUILD_DIR)/kernel.xclbin
...
$(TEMP_DIR)/kernel.xo: src/kernel.cpp
	mkdir -p $(TEMP_DIR)
	v++ -c $(VPP_FLAGS) -t $(TARGET) --platform $(PLATFORM) -k calculate_crc --temp_dir $(TEMP_DIR)  -I'$(<D)' -o'$@' '$<'
